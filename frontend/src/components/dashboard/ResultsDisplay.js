...
function ResultsDisplay({ resultData, resultType }) {
  ...
  // --- RENDER LOGIC FOR A SINGLE SIMULATION ---
  ...
  // --- RENDER LOGIC FOR A COMPARISON SIMULATION ---
  if (resultType === 'comparison') {
    const { individual_results, comparison_analysis, simulation_parameters } = resultData;
    ...
    return (
      <div>
        ...
        {/* Display the main ANOVA results first */}
        ...
        {/* Map over each algorithm's results and render them */}
        {algorithmIds.map((algoId) => (
          <div key={algoId} className="result-group">
            <h3>Algorithm: <span className="algorithm-name">{algoId}</span></h3>
            <StatsTable stats={individual_results[algoId].summary_stats} />
            <ShotDistributionChart histogramData={individual_results[algoId].histogram} />
          </div>
        ))}
      </div>
    );
  ...
  return null; // Fallback in case of unknown result type
}
